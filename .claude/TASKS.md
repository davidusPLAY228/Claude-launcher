...
### ✅ Задача 3: Выдвижное боковое меню (sidebar) — ВЫПОЛНЕНА
**Дата выполнения:** 2026-08-01  
**Статус:** Завершена полностью

**Что сделано:**
1. Создана папка `assets/` с SVG иконками:
   - `icon-menu.svg` — иконка гамбургер-меню (3 горизонтальные линии)
   - `icon-omniroute.svg` — иконка play для запуска OmniRoute
   - `icon-nvm.svg` — иконка загрузки для установки nvm
2. Добавлен sidebar в HTML с двумя пунктами меню
3. Реализована полная стилизация sidebar:
   - Схлопнутое состояние: ширина 52px, видны только иконки
   - Развёрнутое состояние: ширина 200px, видны иконки + текст
   - Плавная анимация через CSS transition (0.25s ease)
   - Кнопка раскрытия с поворотом иконки на 90° при раскрытии
4. Изменён общий layout:
   - `.app` теперь `flex-direction: row` вместо `column`
   - Sidebar слева, `.app-content` справа с `flex: 1`
5. Добавлена логика раскрытия/схлопывания через `sidebar.classList.toggle('expanded')`
6. Реализованы обработчики кнопок:
   - **Запуск OmniRoute** — вызывает `window.api.launchOmniRoute(cfg)` с текущей конфигурацией
   - **Установка nvm** — открывает ссылку через `shell.openExternal`
7. Добавлен IPC обработчик `open:external` в main.js для открытия внешних ссылок

**Изменённые файлы:**
- `assets/` — новая папка с тремя SVG иконками
- `index.html` — добавлен sidebar перед `.app-content`
- `styles.css` — полная стилизация sidebar, изменён layout на row
- `renderer.js` — логика раскрытия и обработчики кнопок sidebar
- `main.js` — добавлен обработчик `ipcMain.handle('open:external', ...)`
- `preload.js` — добавлен метод `openExternal`

**Поведение:**
- По умолчанию sidebar схлопнут (52px), видны только иконки
- При клике на кнопку гамбургера sidebar раскрывается с анимацией (200px)
- Иконка гамбургера поворачивается на 90° при раскрытии
- Текстовые названия плавно появляются/исчезают через opacity
- Кнопка "Запуск OmniRoute" запускает OmniRoute с текущими настройками
- Кнопка "Установка nvm" открывает браузер со ссылкой на скачивание

**Улучшения (09:47):**
- Добавлена динамическая смена иконки кнопки OmniRoute:
  - Если OmniRoute не запущен → иконка **play** (треугольник)
  - Если OmniRoute запущен → иконка **pause** (две вертикальные черты)
- Добавлен диалог при попытке запуска уже работающего OmniRoute:
  - Вместо простого сообщения "уже запущен" показывается `confirm()` с вопросом "OmniRoute уже запущен. Остановить его?"
  - Если пользователь нажимает "Да" → вызывается `stopAll()` для остановки всех процессов OmniRoute
  - Если "Нет" → ничего не происходит
- Автоматическая проверка статуса OmniRoute при загрузке приложения
- Иконка и tooltip кнопки обновляются после каждого действия (запуск/остановка)

**Технические детали:**
- Создана иконка `assets/icon-pause.svg` (две вертикальные черты)
- Добавлен IPC обработчик `check:omniroute-status` в main.js для проверки статуса
- Функция `updateOmniRouteStatus()` проверяет статус и обновляет иконку
- Флаг `omnirouteRunning` отслеживает текущий статус OmniRoute
